...
final nearbyPlacesProvider =
    FutureProvider.family<List<PlaceWithDistance>, double>((
      ref,
      radiusKm,
    ) async {
      final placesAsync = await ref.watch(placesProvider.future);
      final userLocation = await ref.watch(
        currentLocationProvider(LocationAccuracyLevel.medium).future,
      );
...
final closestPlaceProvider = FutureProvider<PlaceWithDistance?>((ref) async {
  final nearbyPlaces = await ref.watch(
    nearbyPlacesProvider(10.0).future,
  ); // 10km radius
