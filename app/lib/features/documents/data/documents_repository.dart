...
class DocumentsRepository {
  ...
  Future<List<Document>> getAllDocuments() async {
    try {
      ...
      final String response = await rootBundle.loadString('assets/fixtures/documents.json');
  ...
  Future<List<Document>> getDocumentsByCategory(DocumentCategory category) async {
    final allDocs = await getAllDocuments();
  ...
  Future<List<Document>> searchDocuments(String query) async {
    ...
    return allDocs.where((doc) {
      ...
      // Search in tags
      if (doc.tags?.any((tag) => tag.toLowerCase().contains(lowerQuery)) == true) return true;
